...
GENERATED_CODE_DIR="src/main/kotlin/generated"

for STRATEGY in "local" "worker"; do
    echo ":: Executing with the [${STRATEGY}] strategy."
    echo "build --strategy=Detekt=${STRATEGY}" > ".bazelrc"

    # Generate a bit of code to keep Bazel working instead of pulling from cache.
    mkdir -p "${GENERATED_CODE_DIR}"
    touch "${GENERATED_CODE_DIR}/generated${STRATEGY}.kt"
