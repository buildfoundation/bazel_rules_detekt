...
`config` | [`Label`](https://docs.bazel.build/versions/master/skylark/lib/Label.html) | `None` | Analysis configuration. If omitted (or None), uses the selected toolchain's default_config. An explicit config replaces the entire default profile; individual options are not merged.
